...
| [TWZRD Agent Intel](https://intel.twzrd.xyz) | Trust-scoring MCP server for multi-agent financial workflows on Solana. Free preflight returns a reputation score for any agent wallet; paid call returns a signed USDC trust receipt settled on-chain in <1s via x402. Streamable-HTTP MCP at `https://intel.twzrd.xyz/mcp` — lets LangGraph agents verify counterparty trust before executing autonomous USDC transactions. | [![GitHub](https://img.shields.io/github/stars/twzrd-sol/wzrd-final?style=social)](https://github.com/twzrd-sol/wzrd-final) |
